...
if (-not (Test-Path .)) { Write-Error "Current directory not found." }
...
$lsRemoteJob = Start-Job -ScriptBlock { param($url) git ls-remote --exit-code $url -ErrorAction SilentlyContinue } -ArgumentList $remoteUrl
$jobCompleted = Wait-Job -Job $lsRemoteJob -Timeout 15
$lsRemoteResult = $null
if ($jobCompleted) {
    $lsRemoteResult = Receive-Job -Job $lsRemoteJob
}
Remove-Job -Job $lsRemoteJob

if (-not $lsRemoteResult) {
...
} elseif (-not $jobCompleted) {
    Write-Warning "git ls-remote timed out after 15 seconds. Please check your network connection."
}
    } else {
        Write-Warning "GitHub repo does not exist and gh CLI not found. Please create the repo manually: $remoteUrl"
    }
